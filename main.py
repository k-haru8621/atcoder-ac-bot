...
SCHEDULE_TIME = time(hour=9, minute=0, tzinfo=JST)
...
class AtCoderBot(discord.Client):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as resp:
                ...
                if news_section:
                    ...
                    if news_list:
                        embeds.append(discord.Embed(title="📢 最近の告知", description="\n".join([f"• {n}" for n in news_list]), color=0x34495e))
                return embeds
    ...
    async def daily_info_task(self):
        ...
        # news_config に登録された全チャンネルへ送信
        for guild_id, ch_id in self.news_config.items():
            channel = self.get_channel(ch_id)
            if channel:
                try: await channel.send("☀️ **本日のAtCoder情報です**", embeds=embeds)
                except: pass
    ...
    async def send_notification(self, info, sub):
        ...
        color = 0xFFFFFF if sub['result'] != 'AC' else self.get_difficulty_color(diff_val)
        dt_jst = datetime.fromtimestamp(sub['epoch_second'], JST)
        time_str = dt_jst.strftime('%Y年%m月%d日 %H:%M:%S')
...
async def register(interaction: discord.Interaction, discord_user: discord.Member, atcoder_id: str, channel: discord.TextChannel, only_ac: bool):
    ...
    unique_key = f"{guild_id}_{atcoder_id}"
    if unique_key in bot.user_data:
        await interaction.response.send_message(f"❌ `{atcoder_id}` は登録済みです。", ephemeral=True)
        return
    bot.user_data[unique_key] = {
        "guild_id": guild_id, "discord_user_id": discord_user.id,
        "atcoder_id": atcoder_id, "channel_id": channel.id,
        "only_ac": only_ac, "registered_at": datetime.now().timestamp()
    }
    bot.save_data(bot.user_data, USER_DATA_FILE)
    mode = "ACのみ" if only_ac else "すべての提出"
...
async def delete(interaction: discord.Interaction, atcoder_id: str):
    ...
    if unique_key not in bot.user_data:
        await interaction.response.send_message("❓ 未登録です。", ephemeral=True)
        return
    if interaction.user.id == bot.user_data[unique_key]["discord_user_id"] or interaction.user.guild_permissions.manage_messages:
        ...
        bot.save_data(bot.user_data, USER_DATA_FILE)
        await interaction.response.send_message(f"🗑️ `{atcoder_id}` を削除しました。")
    else:
        await interaction.response.send_message("❌ 権限がありません。", ephemeral=True)

@bot.tree.command(name="notice_set", description="このサーバーの定時ニュース送信先を設定します")
@app_commands.checks.has_permissions(manage_channels=True)
async def notice_set(interaction: discord.Interaction, channel: discord.TextChannel):
    guild_id = str(interaction.guild_id)
    bot.news_config[guild_id] = channel.id
    bot.save_data(bot.news_config, NEWS_CONFIG_FILE)
    await interaction.response.send_message(f"✅ 定時ニュースの送信先を {channel.mention} に設定しました。")

@bot.tree.command(name="notice_delete", description="定時ニュースの設定を解除します")
@app_commands.checks.has_permissions(manage_channels=True)
